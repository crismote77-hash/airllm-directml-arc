...
from .device_utils import can_pin_memory, supports_bitsandbytes
...
class AirLLMBaseModel:
    ...
    def __init__(self, model_local_path_or_repo_id, device="cuda:0", dtype=None, max_seq_len=512,
        ...
        if compression is not None and not supports_bitsandbytes(device):
            raise ValueError(
                f"Compression ('4bit'/'8bit') requires a CUDA (NVIDIA) device, "
                f"but got device='{device}'. "
                f"DirectML / MPS / CPU devices do not support bitsandbytes compression. "
    ...
    def load_layer_to_cpu(self, layer_name):
        ...
        if self.prefetching and can_pin_memory(self.running_device):
